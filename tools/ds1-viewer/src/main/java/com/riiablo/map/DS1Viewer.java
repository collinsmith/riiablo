...
public class DS1Viewer extends ApplicationAdapter {
  ...
  public void create() {
    ...
    final VisSelectBox<LvlPrest.Entry> presets = new VisSelectBox<LvlPrest.Entry>() {{
      ...
      for (LvlPrest.Entry row : Riiablo.files.LvlPrest) {
        presets.add(row);
      }
    ...
    variations.addListener(new ChangeListener() {
      ...
      public void changed(ChangeEvent event, Actor actor) {
        ...
        if (mapDescriptor != null) Riiablo.assets.unload(mapDescriptor.fileName);

        com.riiablo.map.DS1Types.Entry ds1Type = DS1Types.get(preset.Def);
        LvlTypes.Entry lvlType = Riiablo.files.LvlTypes.get(ds1Type.LevelType);
        ...
        Riiablo.assets.load(mapDescriptor);
        Riiablo.assets.finishLoadingAsset(mapDescriptor);
        ...
        palette = Palette.loadFromFile(Riiablo.mpqs.resolve(PALETTES[lvlType.Act - 1])).render();
      }
    });
    ...
    Batch batch = Riiablo.batch = new PaletteIndexedBatch(2048, shader);
