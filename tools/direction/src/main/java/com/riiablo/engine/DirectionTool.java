...
public class DirectionTool extends Tool {
  ...
  public void create() {
    ...
    iso.offset(0, -DT1.Tile.SUBTILE_HEIGHT50);
    iso.set(0, 0);
    iso.update();
    tmpVec2a.set(iso.position);
    ...
    Gdx.input.setInputProcessor(new InputAdapter() {
      ...
      void update(int screenX, int screenY) {
        iso.agg(tmpVec2b.set(screenX, screenY)).unproject().toWorld();
        x2 = screenX;
        y2 = height - screenY;
      }
    });

    print(Direction.RADIANS_8);
    print(Direction.RADIANS_8M);
  }
