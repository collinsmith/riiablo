package com.riiablo.mpq_bytebuf.util;

import io.netty.buffer.ByteBuf;
import java.util.Map;
import java.util.TreeMap;

import com.riiablo.logger.LogManager;
import com.riiablo.logger.Logger;

public final class Huffman {
  private static final Logger log = LogManager.getLogger(Huffman.class);

  private static final short[][] CODES = {
      /* data for compression type 0x00. */
      {
          , // originally 0x00, 0x00
      },

      {
          /* data for compression type 0x01. */
          , // originally 0x00, 0x00
      },

      {
          /* data for compression type 0x02. */
          , // originally 0x00, 0x00
      },

      {
          /* data for compression type 0x03. */
          , // originally 0x00, 0x00
      },

      {
          /* data for compression type 0x04. */
          , // originally 0x00, 0x00
      },

      {
          /* data for compression type 0x05. */
          , // originally 0x00, 0x00
      },

      {
          /* data for compression type 0x06. */
          , // originally 0x00, 0x00
      },

      {
          /* data for compression type 0x07. */
          , // originally 0x00, 0x00
      },

      {
          /* data for compression type 0x08. */
          , // originally 0x00, 0x00
      },
  };

  static final class Node {
    Node parent;
    Node next;
    Node prev;
    int code;
    int freq;
    Node left, right;

    void treeSwap(Node with) {
      Node temp;

      if (parent == with.parent) {
        temp = parent.left;
        parent.left = parent.right;
        parent.right = temp;
      } else {
        if (with.parent.left == with) with.parent.left = this;
        else with.parent.right = this;
        if (this.parent.left == this) this.parent.left = with;
        else this.parent.right = with;
      }

      temp = parent;
      parent = with.parent;
      with.parent = temp;
    }

    void insertAfter(Node where) {
      prev = where;
      next = where.next;
      where.next = this;
      next.prev = this;
    }

    void listSwap(Node with) {
      if (next == with) {
        next = with.next;
        with.next = this;
        with.prev = prev;
        prev = with;

        with.prev.next = with;
        next.prev = this;
      } else if (prev == with) {
        prev = with.prev;
        with.prev = this;
        with.next = next;
        next = with;

        with.next.prev = with;
        prev.next = this;
      } else {
        Node temp = prev;
        prev = with.prev;
        with.prev = temp;

        temp = next;
        next = with.next;
        with.next = temp;

        prev.next = this;
        next.prev = this;

        with.prev.next = with;
        with.next.prev = with;
      }
    }

    void newList() {
      prev = next = this;
    }

    Node removeFromList() {
      if (this == next) return null;

      prev.next = next;
      next.prev = prev;

      return next;
    }

    void joinList(Node list) {
      Node tail = prev;

      prev = list.prev;
      prev.next = this;

      list.prev = tail;
      tail.next = list;
    }
  }

  private Node nodes = null;
  private TreeMap<Integer, Node> sorted2 = new TreeMap<>();

  private Node root = null;

  private int bitBuffer;
  private byte bitNumber;
  private ByteBuf source;

  private void setSource(ByteBuf source) {
    this.source = source;
    bitBuffer = 0;
    bitNumber = 0;
  }

  private int read1() {
    if (bitNumber == 0) {
      bitBuffer = source.readUnsignedByte();
      bitNumber = 8;
    }

    int result = bitBuffer & 0x01;
    bitBuffer >>>= 1;
    bitNumber--;
    return result;
  }

  private int read8() {
    if (bitNumber < 8) {
      bitBuffer |= source.readUnsignedByte() << bitNumber;
      bitNumber += 8;
    }

    int result = bitBuffer & 0xff;
    bitBuffer >>>= 8;
    bitNumber -= 8;
    return result;
  }

  private Node getNode() {
    Node node;
    if (nodes == null) node = new Node();
    else {
      node = nodes;
      nodes = nodes.removeFromList();
    }
    return node;
  }

  private void destroyTree(Node root) {
    if (nodes == null) nodes = root;
    else nodes.joinList(root);
    this.root = null;
    sorted2.clear();
  }

  private void insertNode(Node node) {
    Map.Entry<Integer, Node> test2 = sorted2.ceilingEntry(node.freq);
    Node current;

    if (test2 != null) {
      current = test2.getValue();
      node.insertAfter(current);
    } else {
      if (root != null) {
        node.insertAfter(root.prev);
      } else {
        node.newList();
      }
      root = node;
    }

    sorted2.put(node.freq, node);
  }

  private Node addValueToTree(int value) {
    // create leaf node
    Node node = getNode();
    node.code = value;
    node.freq = 0;
    node.left = null;
    node.right = null;

    insertNode(node);

    // create branch node
    Node node2 = getNode();
    Node child1 = root.prev;
    Node child2 = child1.prev;

    node2.code = -1;
    node2.freq = child1.freq + child2.freq;
    node2.left = child1;
    node2.right = child2;
    node2.parent = child2.parent;

    node2.insertAfter(child2.prev);

    // insert into tree
    if (node2.parent.left == child2) node2.parent.left = node2;
    else node2.parent.right = node2;

    child1.parent = node2;
    child2.parent = node2;

    return node;
  }

  private void incrementProbability(Node node) {
    while (node != null) {
      // possible optimization here. Is all this really nescescary to enforce order?
      if (sorted2.get(node.freq) == node) {
        if (node.freq == node.prev.freq)
          sorted2.put(node.freq, node.prev);
        else
          sorted2.remove(node.freq);
      }
      node.freq += 1;

      Map.Entry<Integer, Node> test2 = sorted2.ceilingEntry(node.freq);
      Node where;
      if (test2 != null) where = test2.getValue().next;
      else where = root;

      if (where != node) {
        node.listSwap(where);
        node.treeSwap(where);

        if (where.freq != where.next.freq) {
          sorted2.put(where.freq, where);
        }
      }
      sorted2.put(node.freq, node);

      node = node.parent;
    }
  }

  private void buildTree(int tree) {
    final short[] probabilities = CODES[tree];
    if (root != null) destroyTree(root);

    // generate leaves
    for (int i = 0; i < 0x102; i++) {
      int prob = probabilities[i];

      if (prob == 0) continue;

      Node node = getNode();
      node.code = i;
      node.freq = prob;
      node.left = null;
      node.right = null;

      insertNode(node);
    }

    // generate tree
    Node current = root.prev;
    while (current != root) {
      Node node = getNode();
      Node child1 = current;
      Node child2 = current = current.prev;

      child1.parent = node;
      child2.parent = node;

      node.code = -1;
      node.freq = child1.freq + child2.freq;
      node.left = child1;
      node.right = child2;
      insertNode(node);

      current = current.prev;
    }

    root.parent = null;
  }

  public int inflate(ByteBuf in, ByteBuf out) {
    log.traceEntry("inflate(in: {}, out: {})", in, out);
    setSource(in);
    int type = read8();
    buildTree(type);

    boolean adjustProbability = type == 0;

    for (;;) {
      Node current = root;
      while (current.code == -1) {
        final int i = read1();
        current = i == 0 ? current.left : current.right;
      }

      if (current.code == 0x101) {
        int value = read8();
        current = addValueToTree(value);
        incrementProbability(current);
        if (!adjustProbability) incrementProbability(current);
      } else if (current.code == 0x100) {
        break;
      }

      out.writeByte(current.code);

      if (adjustProbability) {
        incrementProbability(current);
      }
    }

    return out.writerIndex();
  }
}
