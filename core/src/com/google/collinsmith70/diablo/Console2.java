...
public class Console2 implements Disposable {
...
private List<String> prefixedKeys;
...
public Console2(Client client) {
    ...
    CARET_BLINK_TASK = new Timer.Task() {
        @Override
        public void run() {
            Console2.this.showCaret = !Console2.this.showCaret;
        }
    };
...
private void updateCaret(boolean updateLookup) {
    ...
    if (prefixedKeysIterator == null || updateLookup) {
        ...
        prefixedKeys = ImmutableList.copyOf(autoComplete.keySet());
        prefixedKeysIterator = prefixedKeys.listIterator();
...
public void render(Batch b) {
    ...
    position = getClient().getVirtualHeight() - font.getLineHeight();
    for (ListIterator<String> it = prefixedKeys.listIterator(); it.hasNext(); ) {
        String s = it.next();
        GlyphLayout gl = font.draw(b, s, 0.0f, position);
        if (s.equals(currentlyReadKey)) {
            b.draw(highlightBackgroundTexture, 0.0f, position - font.getXHeight() - font.getAscent(), gl.width, font.getCapHeight());
            font.draw(b, s, 0.0f, position);
        }
        position -= font.getLineHeight();
    }
}
